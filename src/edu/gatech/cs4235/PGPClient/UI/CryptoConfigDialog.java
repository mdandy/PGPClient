...
import edu.gatech.cs4235.PGPClient.DAL.RemoteClientKey;
 ...
 * @author mdandy
 */
public class CryptoConfigDialog extends JDialog 
{
	 ...
	 * Create the dialog.
	 */
	public CryptoConfigDialog(JFrame frame)
	{
		/* Set Properties */
		setResizable(false);
		setAlwaysOnTop(true);
		setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);
		setTitle("PGPClient - Crypto Config");
	...
	private void updateTable()
	{
		...
		ArrayList<RemoteClientKey> list = (ArrayList<RemoteClientKey>) or.getKeyStore();
	...
	private class ButtonListener implements ActionListener
	{
		private void updateTable(String username, String userPublicKey)
		{
			...
			if (publickey == null)
			{
				JOptionPane.showMessageDialog(CryptoConfigDialog.this, "Error: Unable to parse key!", "Error", JOptionPane.ERROR_MESSAGE);
			}
		...
		private void saveKeyManagement(String privateKey, String publicKey, String passphrase)
		{
			...
			boolean successful = FileIO.write(filepath, privateKey);
			if (!successful)
			{
				JOptionPane.showMessageDialog(CryptoConfigDialog.this, "Error: Unable to write Private Key to a file!", "Error", JOptionPane.ERROR_MESSAGE);
			}

			filepath = pref.getKeyDirectory() + File.separator + pref.getPublicKeyFilePath();
			successful = FileIO.write(filepath, publicKey);
			if (!successful)
			{
				JOptionPane.showMessageDialog(CryptoConfigDialog.this, "Error: Unable to write Public Key to a file!", "Error", JOptionPane.ERROR_MESSAGE);
			}
		...
		public void actionPerformed(ActionEvent e) 
		{
			...
			if (source == btnKeyDirectoryBrowse)
			{
				...
				if (FileIO.isFileOrDirectoryExist(txtKeydirectory.getText()))
				{
					browser.setCurrentDirectory(new File (txtKeydirectory.getText()));
				}
				int returnVal = browser.showOpenDialog(CryptoConfigDialog.this);
			...
			else if (source == btnUserKeyAdd)
			{
				...
				String userPublicKey = FileIO.read(txtUserkey.getText());
				if (userPublicKey == null)
				{
					JOptionPane.showMessageDialog(CryptoConfigDialog.this, "Error: Unable to read the file!", "Error", JOptionPane.ERROR_MESSAGE);
				}
				else
				{
			...
			else if (source == btnApply)
			{
				...
				else if (component == panelKeyManagement)
				{
					...
					{
						JOptionPane.showMessageDialog(CryptoConfigDialog.this, "Error: Passphrases do not match!", "Error", JOptionPane.ERROR_MESSAGE);
					}
					else
					{
			...
			else if (source == okButton)
			{
				...
				/* Close the dialog */
				CryptoConfigDialog.this.dispose();
			}
